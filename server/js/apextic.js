 ...
 *  14-07-2019 - adapted for plug-in V2
 *  15-07-2019 - bugfixes           
 *               - overwriting existing values
 *               - bug in AM/PM calculation fixed
 *               - sizing and positioning on phone fixed
 *               - floating labels reacting
 * 
 */
...
{ 
  /* default both buttons are black */
  let buttonAMcolor = 'black';
  let buttonPMcolor = 'black';
  
  /* determine the color of the buttons */
  this.mode = theMode;
  if ( this.mode == 'AM' )
  { 
    this.window_base = 0;
  ...
  $('#buttonAM text').attr('fill',buttonAMcolor);
  $('#buttonPM text').attr('fill',buttonPMcolor);
...
{ 
  /* show the clock control */
  $('#'+this.ticId).show();
  /* hide the hands */
  apextic.hideHands();
  /* initialize the am/pm buttons */
  apextic.setMode();  
},
...
{ 
  ...
  svgHeight = $(window).height() - 70;
  
  svg = this.svgCreateRootElem("ticClock",svgWidth,svgHeight,"0 0 350 350");
  $(svg).appendTo( $('#ticBox') );
  ...
  { $("[id^=txt]").click ( function (event) {
       ...
       { 
         ...
         if ( apextic.mode == 'PM' && apextic.currentHour < 12 ) 
...
doIt : function() 
{
    ...
    /* 
     * create time picker icons
     * attach click event to show the clock
     */
    $('.has-time-picker').each( function() { 
        var item_id = $(this).attr('id');
        $(this).after(`<div class="a-Button a-Button-inline" onclick="apextic.ticShow('#${item_id}',${window_base},'${time_format}')"><i class="fa fa-clock-o"></i></div>`); 
    } );
